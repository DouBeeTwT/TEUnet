...
class TEUnet2_Encoder(nn.Module):
    def __init__(self,
        ...
        self.DownSampleLayer2 = nn.Sequential(
            ...
            *[DoubleConv2d(in_channels=hidden_channels*2, out_channels=hidden_channels*2, p=p) for _ in range(2)]
            )
        self.DownSampleLayer3 = nn.Sequential(
            DoubleConv2d(in_channels=hidden_channels*2, out_channels=hidden_channels*4, stride=2, p=p),
            *[DoubleConv2d(in_channels=hidden_channels*4, out_channels=hidden_channels*4, p=p) for _ in range(3)]
            )
        self.DownSampleLayer4 = nn.Sequential(
            DoubleConv2d(in_channels=hidden_channels*4, out_channels=hidden_channels*8, stride=2, p=p),
            *[DoubleConv2d(in_channels=hidden_channels*8, out_channels=hidden_channels*8, p=p) for _ in range(5)]
            )
        self.DownSampleLayer5 = nn.Sequential(
            DoubleConv2d(in_channels=hidden_channels*8, out_channels=hidden_channels*16, stride=2, p=p),
            *[DoubleConv2d(in_channels=hidden_channels*16, out_channels=hidden_channels*16, p=p) for _ in range(2)]
            )
